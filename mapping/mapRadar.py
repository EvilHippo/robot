...
"""
Try to fix the angle before turning
"""
def fixAngleOld(robot):
    # TODO COMPLETLY FAILS, FIX.
    return
    start = robot.gyro.value()
    print("\nIn fix angle, start: ", start)

    next = 0
    while((start + next) % 90 != 0):
        next += 1

    prev = 0
    while((start + prev) % 90 != 0):
        prev -= 1
    if (prev == 0 == next):
        return
    ...
    if(next < abs(prev) ):
        print("Fixing by: ", next)
        robot.turnSameSpot("right")
        while(robot.gyro.value() < start + next):
            pass
    else:
        print("Fixing by: ", prev)
        robot.turnSameSpot("left")
        while (robot.gyro.value() > start + prev):
            pass
    print("New gyro: ", robot.gyro.value())
    print()
    robot.stop()
    time.sleep(5)
...
def fixAngle(robot):
    return
    start = robot.motor_right.position
    print("\nIn fix angle, start: ", robot.gyro.value())
    print("Position(R): ", robot.motor_right.position, "  ,%190: ", robot.motor_right.position % 190)
    print("Position(L)", robot.motor_left.position, "  ,%190: ", robot.motor_left.position % 190)

    next = 0
    while ((start + next) % 190 != 0):
        next += 1

    prev = 0
    while ((start + prev) % 190 != 0):
        prev -= 1
    #if (prev == 0 == next):
     #   print("All is fine")
      #  return
    ...
    if (next < abs(prev)):
        ...
        #robot.turnSameSpot("right")
        robot.turnSameSpot("left")
        while (robot.motor_right.position < start + next) and not robot.btn.any():
            print(robot.motor_right.position)
            pass
    else:
        print("Fixing by: ", prev)
        #robot.turnSameSpot("left")
        robot.turnSameSpot("right")
        while (robot.motor_right.position > start + prev) and not robot.btn.any():
            print(robot.motor_right.position)
            pass
    print("New gyro: ", robot.gyro.value())
    print("\n")
...
def tryTurnNow(robot, rightSonar, leftSonar):
    ...
    if (rightSonar > 150 and rightSonar != -1):
        robot.turnSameSpot("right")
        ...
        robot.stop()
        return "right"
    elif (leftSonar > 200 and leftSonar != -1):
        robot.turnSameSpot("left")
...
def makeMap(robot):
    ...
    while(not robot.btn.any()):
        ...
        if(time.time() - robot.timeToCross >= start_time):
            ...
            start_time = time.time()
            robot.drive()
            adjust(robot, rightSonar, leftSonar, wallCloseSide)
