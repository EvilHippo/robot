    ...
    def driveClose(self):
        ...
        while not self.btn.any():
            ...
            if (abs(sonar - lastS) > 200) and ((targetLost is False and lastS < startValue) or (sonar > startValue and targetLost is False)):
                targetLost = True
                lastSeen = lastS
                print("driveClose: TARGET LOST, sonar: ", sonar, "   other sonar: ", sonar2)
            if (targetLost is True):
                self.stop()
                self.turnSameSpot("left")
                while(not(sonar - 20 <= lastSeen and sonar + 20 >= lastSeen) and sonar > startValue or sonar < 200):
                    sonar = self.sonar.value()
                self.stop()
                print("driveClose: TARGET FOUND, sonar: ", sonar, "   other sonar: ", sonar2)
                self.drive()
                targetLost = False
...
def main():
    ...
    while(findThree < 2 and not robot.btn.any()):
        ...
        while(sonar - 10 <= distances[findThree] and sonar + 10 >= distances[findThree]):
            sonar = robot.sonar.value()
