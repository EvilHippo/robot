...
class Robot:
    ...
    def turnNew(self, dir):
        # print("********************************* IN NEW TURN *************************")
        if (dir == "right"):
            self.motor_left.run_timed(time_sp=100000, speed_sp=150)
            self.motor_right.run_timed(time_sp=100000, speed_sp=50)

        if (dir == "left"):
            self.motor_right.run_timed(time_sp=100000, speed_sp=150)
            self.motor_left.run_timed(time_sp=100000, speed_sp=50)
...
def main():
    ...
    while not robot.btn.any():
        ...
        if(not extra_black):
            ...
            # came across a crossing
            if (last_turn == "right" and bCount >0):
                extra_black = True
            elif (last_turn == "left" and bCount > 1):
                robot.stop()
                bCount = 0
                print("GOING RIGHT")
                robot.turnNew("right")
            ...
            elif (last_turn == "right" and count >= countTo):
                robot.stop()
                robot.turnNew("left")
                last_turn = "left"
            elif bCount >= countTo:
